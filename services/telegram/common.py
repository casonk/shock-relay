...
def normalize_updates_response(result: Any) -> Dict[str, Any]:
    ...
    update_ids = [
        update["update_id"] for update in normalized if isinstance(update.get("update_id"), int)
    ]
...
def normalize_update(update: Any) -> Dict[str, Any]:
    ...
    chat_payload = (
        message_payload.get("chat") if isinstance(message_payload.get("chat"), dict) else {}
    )
    from_payload = (
        message_payload.get("from") if isinstance(message_payload.get("from"), dict) else {}
    )
